...
<asp:GridView id="gvApplications" runat="server" AutoGenerateColumns="False" AllowPaging="true" 
	ShowHeader="false" CssSelectorClass="NormalGridView" EmptyDataText="gvApplications" OnRowCommand="gvApplications_RowCommand" 
			...
			<ItemTemplate>
				<div style="text-align:center;">
					<asp:hyperlink NavigateUrl='<%# EditUrl("ApplicationID", Eval("Id").ToString(), "edit", "SpaceID=" + PanelSecurity.PackageId.ToString()) %>'
						...
						<asp:Image runat=server ID="Image1" ImageUrl='<%# "~/DesktopModules/WebsitePanel/ResizeImage.ashx?width=150&height=150&url=" + Server.UrlEncode((string)Eval("IconUrl"))  %>'
